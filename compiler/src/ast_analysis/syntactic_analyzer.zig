...
pub const SyntacticAnalyzer = struct {
    ...
    variable_mutability: std.StringHashMap(bool),
    variable_references: std.StringHashMap(bool),
    ...
    pub fn init(allocator: Allocator) SyntacticAnalyzer {
        return SyntacticAnalyzer{
            ...
            .variable_mutability = std.StringHashMap(bool).init(allocator),
            .variable_references = std.StringHashMap(bool).init(allocator),
    ...
    pub fn deinit(self: *SyntacticAnalyzer) void {
        ...
        self.variable_mutability.deinit();
    ...
    fn analyze_declaration(self: *SyntacticAnalyzer, decl: AST.Declaration) std.mem.Allocator.Error!void {
        ...
        // Track variable type and mutability
        ...
        // Track variable mutability and reference status
        const is_mutable = decl.kind == .var_decl;
        try self.variable_mutability.put(decl.name, is_mutable);
        
        // Check if this is a reference variable (type starts with &)
        const is_reference = if (decl.type_annotation) |type_ann| 
            type_ann == .reference
        else false;
        try self.variable_references.put(decl.name, is_reference);
        ...
        switch (expr) {
            ...
            .binary_op => |binop| {
                ...
                if (std.mem.eql(u8, binop.operator, "=")) {
                    try self.validate_assignment(binop);
                }
    ...
    fn validate_assignment(self: *SyntacticAnalyzer, assignment: AST.BinaryOp) std.mem.Allocator.Error!void {
        // Check if left side is an identifier (variable assignment)
        if (assignment.left.* == .identifier) {
            const var_name = assignment.left.*.identifier;
            
            // Check if variable is a reference
            if (self.variable_references.get(var_name)) |is_reference| {
                if (is_reference) {
                    const msg = try std.fmt.allocPrint(self.allocator, "Cannot assign to reference variable '{s}'", .{var_name});
                    try self.errors.append(SyntacticError{
                        .message = msg,
            ...
            if (self.variable_mutability.get(var_name)) |is_mutable| {
                if (!is_mutable) {
                    const msg = try std.fmt.allocPrint(self.allocator, "Cannot assign to immutable variable '{s}'", .{var_name});
                    try self.errors.append(SyntacticError{
                        .message = msg,
                        .node_type = "assignment",
                        .severity = .err,
